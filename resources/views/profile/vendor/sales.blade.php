    ...
    <table class="table table-hover table-striped">
        ...
        @foreach($sales as $purchase)
            <tr>
                ...
                <td class="text-right">
                    <span class="badge badge-dark">@include('includes.currency', ['usdValue' => $purchase -> value_sum])</span>
                </td>
                <td>
                    <input type="text" readonly="readonly" class="form-control" value="{{ $purchase -> address }}">
                </td>
                <td>
                    <a href="{{ route('profile.sales.single', $purchase) }}" class="btn btn-sm {{ $purchase -> isCanceled() ? 'btn-danger' : 'btn-dark' }}">
