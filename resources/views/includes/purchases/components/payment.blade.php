    ...
    <table class="table ">
        <tr>
            ...
            <td>
                ...
                @else
                    {{ $purchase -> coin_sum }} <span class="badge badge-light">{{ $purchase -> coin_label }}</span>
                @endif
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                @else
                    ...
                    @else
                        {{ $purchase -> coin_balance }} <span class="badge badge-light">{{ $purchase -> coin_label }}</span>
                    @endif
        ...
        <tr>
            ...
            <td>
                <div class="btn-group">
                    <span class="btn disabled btn-sm @if($purchase -> isPurchased()) btn-dark @else btn-outline-secondary @endif">Purchased</span>
                    @if($purchase->type=='normal')
                    <span class="btn disabled btn-sm @if($purchase -> isSent()) btn-dark @else btn-outline-secondary @endif">Sent</span>
                    @endif
                    <span class="btn disabled btn-sm @if($purchase -> isDelivered()) btn-dark @else btn-outline-secondary @endif">Delivered</span>
            ...
            <td colspan="2" class="justify-content-center text-center">
                @if($purchase->isPurchased())
                    <a href="{{ route('profile.purchases.canceled.confirm', $purchase) }}"
                       class="btn btn-outline-danger"><i class="fa fa-window-close mr-1"></i> Cancel purchase</a>
                @endif


                @if($purchase->type == 'normal' && $purchase -> isPurchased() && $purchase -> isVendor())
                    <a href="{{ route('profile.sales.sent.confirm', $purchase) }}"
                       class="btn btn-outline-mblue"><i class="fa fa-clipboard-check mr-2"></i> Mark as
                        sent</a>
                @endif

                 @if($purchase->type == 'normal' && $purchase -> isSent() && $purchase -> isBuyer())
                    <a href="{{ route('profile.purchases.delivered.confirm', $purchase) }}"
                       class="btn btn-outline-success"><i class="fa fa-clipboard-check mr-2"></i> Mark as
                        delivered</a>
                @endif


                @if(!$purchase -> isDisputed() && ($purchase -> isBuyer() || $purchase -> isVendor()))
                    <a href="#dispute" class="btn btn-outline-danger"><i class="fa fa-poop mr-2"></i>
                        Dispute</a>
                @endif
    ...
    @if($purchase -> isPurchased() && $purchase -> isBuyer() && !$purchase -> enoughBalance())
        <div class="alert alert-warning text-center">
            To proceed with purchase send the enough <em>Bitcoin</em> to the address: <span
                    class="badge badge-light">{{ $purchase -> address }}</span>
        </div>
    @endif
