...
<form action="{{ route('profile.vendor.product.images.post', $basicProduct) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-inline">
        ...
        <button type="submit" class="btn btn-dark mb-2">Add image</button>
    </div>
</form>
...
<div class="card-columns">
    @foreach($productsImages as $image)
        <div class="card my-3 p-2 @if($image -> first) bg-success @endif">
            <img class="card-img" src="{{ asset('storage/products/' . $image -> image) }}" alt="Product image">
            <div class="card-img-overlay text-center">
                @if(!$image -> first)
                    <a href="{{ route('profile.vendor.product.images.default', $image -> id) }}" class="btn btn-sm btn-dark">Default</a>
                    <a href="{{ route('profile.vendor.product.images.remove', $image -> id) }}" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a>
                @else
