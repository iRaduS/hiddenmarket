...
<form method="POST" action="{{ route('profile.vendor.product.offers.add', $basicProduct) }}">
    ...
    <div class="form-row">
        ...
        <div class="col">
            <button class="btn btn-outline-success" type="submit"><i class="fa fa-plus mr-2"></i> Add offer</button>
        </div>
    </div>
</form>
...
<div class="col-md-12 text-center mt-3">
    @if(request() -> is('profile/vendor/product/edit/*'))
        <a href="{{ route('profile.vendor.product.edit', [$basicProduct, $basicProduct -> afterOffers()]) }}" class="btn btn-outline-dark"><i class="fa fa-chevron-down mr-2"></i>  Next</a>
    @elseif(request() -> is('admin/product/*'))
        <a href="{{ route('admin.product.edit', [$basicProduct, $basicProduct -> afterOffers()]) }}" class="btn btn-outline-dark"><i class="fa fa-chevron-down mr-2"></i>  Next</a>
    @else
        <a href="{{ route('profile.vendor.product.' . ( session('product_type') == 'physical' ? 'delivery' : 'digital' ) ) }}" class="btn btn-outline-dark"><i class="fa fa-chevron-down mr-2"></i>  Next</a>
    @endif
</div>
@stop