                ...
                <div class="slides">
                    ...
                    @foreach($product -> images() -> orderBy('first', 'desc') -> get() as $image)
                            <div id="slide-{{ $i++ }}">
                                ...
                                @else
                                    <img src="{{ asset('storage/products/product.png') }}">
                                @endif
                            </div>
                    @endforeach
                </div>
                ...
                <div class="col-md-12 text-center">
                    ...
                    <table class="table border-0 text-left table-borderless">
                        ...
                        <tr>
                            ...
                            <td>
                                <strong class="badge badge-light">{{ ucfirst($product -> type) }}</strong>
                            </td>
                        </tr>
                        <tr>
                            <td class="text-right text-muted">
                            ...
                            <td colspan="2">
                                @if($product->user->vendor->experience < 0)
                                    <p class="text-danger border border-danger rounded p-1 mt-2"><span
                                                class="fa fa-exclamation-circle"></span> Negative experience, trade with caution !
                                    </p>
                                @endif
                            </td>
                        </tr>
                        ...
                        <tr class="bg-light">
                            ...
                            <td>
                                ...
                                @elseif(count($product -> getCoins()) == 1)
                                    <span class="badge badge-dark">{{ strtoupper(\App\Purchase::coinDisplayName($product -> getCoins()[0])) }}</span>
                        ...
                        <tr class="bg-light">
                            ...
                            <td>
                                ...
                                @elseif(count($product -> getTypes()) == 1)
                                    <span class="badge badge-dark">{{ \App\Purchase::$types[$product -> getTypes()[0]] }}</span>
                            ...
                            <td class="row">
                                <div class="col-md-5">
                                    <input type="number" min="1" name="amount" id="amount"
                                           ...
                                           class="@if($errors -> has('amount')) is-invalid @endif form-control form-control"
                                           placeholder="Amount of {{ str_plural($product -> mesure) }}"/>
                                </div>
                                <div class="col-md-7">
                                    <button class="btn btn-sm btn-block mb-2 btn-dark"><i class="fa fa-plus mr-2"></i>Add to
                                        cart
                                    </button>
                                    @auth
                                        @if(auth() -> user() -> isWhishing($product))
                                            <a href="{{ route('profile.wishlist.add', $product) }}"
                                               class="btn btn-outline-secondary btn-block btn-sm"><i class="fa fa-heart"></i> Remove
                                                from
                                                wishlist</a>
                                        @else
                                            <a href="{{ route('profile.wishlist.add', $product) }}"
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="btn-group">
                        ...
                        <span class="btn btn-dark active btn-sm">Level {{$product->user->vendor->getLevel()}}</span>
                    </div>
                    ...
                    <div class="row my-1 text-md-center">
                        <div class="col-4">
                            <span class="fa fa-plus-circle text-indigo"></span> {{$vendor->vendor->countFeedbackByType('positive')}}
                        </div>
                        <div class="col-4">
                            <span class="fa fa-stop-circle text-secondary"></span> {{$vendor->vendor->countFeedbackByType('neutral')}}

                        </div>
                        <div class="col-4">
                            <span class="fa fa-minus-circle text-danger"></span> {{$vendor->vendor->countFeedbackByType('negative')}}
                        </div>
                    </div>
                    <hr>
                        <a href="{{ route('profile.messages').'?otherParty='.$product -> user ->username}}" class="btn mb-1 btn-outline-secondary"><span class="fa fa-envelope"></span> Send message</a>
                        <a href="{{ route('search',['user'=>$product->user->username]) }}"  class="btn mb-1 btn-outline-indigo">Seller's products ({{$product -> user ->products()->count()}})</a>
