                ...
                <tbody>
                    @foreach($items as $productId => $item)
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="badge badge-light">{{ strtoupper(\App\Purchase::coinDisplayName($item -> coin_name)) }}</span>
                            </td>
                            <td class="text-center">
                                <span class="badge badge-success">{{ \App\Purchase::$types[$item->type] }}</span>

                                @if($item -> shipping)
                                    {{ $item -> shipping -> name }} -
                                    @include('includes.currency', ['usdValue' => $item -> shipping -> price])
                                @else
                                    <span class="badge badge-light">Digital delivery</span>
                                @endif
                            </td>
                            <td class="text-center">
                                <span class="badge badge-dark">
                            ...
                            <td>
                                @if($item -> message)
                                    ...
                                    @endif
                                @else
                                    <span class="badge badge-light">No message</span>
                                @endif
